...
def __get_engine_string(database: Dict[str, Any]) -> str:
    ...
    sqlite_file: Any = database.get("sqlite")

    if database in [None, {}]:
        return ""

    if database.get("sqlite") is not None:
    ...
    if database["db_port"] is not None:
        retval = f"{retval}:" + database["db_port"]

    retval = f"{retval}/" + database["db_name"]
...
def __get_json_config_files(directory: str) -> List[str]:

    curr_dir = os.path.join(".", directory)

    if Path(curr_dir).is_dir() is False:
        config_dir = os.path.join(".", "better_crypto", directory)
    else:
    ...
    return list(glob.glob(json_filter, recursive=True))
...
def __get_env_cfg_dict(json_cfg_dict: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if env_config_key is not None:
        ...
        return __get_dot_env_dict(dotenv, env_config_key=env_config_key)
    return {}
